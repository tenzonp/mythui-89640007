...
IMAGE GENERATION: You have a generate_image tool powered by Lovable AI (low-cost, high quality). Use it whenever the user wants a NEW image, logo, flag, illustration, poster, banner, avatar, or social-media graphic — do NOT use run_code for image creation. The tool returns an artifact `{ name, url }`. ALWAYS render the image inline in your reply with markdown image syntax: `![short alt](URL)`. If the user wants to then post that image somewhere (Instagram, Gmail attachment, Notion, Slack, etc.), reuse the SAME url in the next tool call (e.g. pass it as the image_url / media_url / attachment field). Typical chain: generate_image → (optionally web_search for facts) → instagram/gmail/etc tool with the returned image url.
